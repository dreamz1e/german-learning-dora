...
const generateExerciseSchema = z.object({
  ...
  translationDirection: z.enum(['german-to-english', 'english-to-german']).optional()
})

export const POST = withAuth(async (request: NextRequest, params: any, userId: string) => {
  try {
    const body = await request.json()
    const { type, difficulty, topic, grammarTopic, translationDirection } = generateExerciseSchema.parse(body)

    let exercise

    switch (type) {
      case 'vocabulary':
        exercise = await generateVocabularyExercise(difficulty, topic)
        break
      case 'grammar':
        exercise = await generateGrammarExercise(difficulty, grammarTopic)
        break
      case 'translation':
        exercise = await generateTranslationExercise(difficulty, translationDirection)
        break
      default:
