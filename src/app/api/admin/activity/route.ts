...
async function getActivityHandler(request: NextRequest, params: {}, userId: string) {
  try {
    ...
    const exerciseActivities = (activityType === 'all' || activityType === 'exercise') ? 
    ...
    const achievementActivities = (activityType === 'all' || activityType === 'achievement') ?
    ...
    const dailyChallengeActivities = (activityType === 'all' || activityType === 'daily_challenge') ?
      await prisma.userDailyChallengeCompletion.findMany({
        where: userIdFilter ? { userId: userIdFilter } : {},
        select: {
          id: true,
          date: true,
    ...
    const activities = [
      ...
      ...dailyChallengeActivities.map(activity => ({
        id: activity.id,
    ...
    const paginatedActivities = activityType === 'all' 
